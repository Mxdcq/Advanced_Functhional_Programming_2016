...
searchB s1 (Node s leftTree rightTree)
  | allElem s1 s  = (searchB s1 leftTree) `par` ((searchB s1 rightTree) `pseq` ((searchB s1 leftTree) ++ [s] ++ (searchB s1 rightTree)))
  | otherwise = (searchB s1 leftTree) `par` ((searchB s1 rightTree) `pseq` ((searchB s1 leftTree) ++ (searchB s1 rightTree)))
